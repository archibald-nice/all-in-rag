...
```mermaid
flowchart LR
    subgraph Step1["步骤1: 数据准备与清洗"]
        A1[多源异构数据<br/>PDF/Word/网页] --> A2[数据标准化]
        A2 --> A3[文本分块<br/>按语义段落切分]
        A3 --> A4[清洗后的<br/>文本块]
    end

    subgraph Step2["步骤2: 索引构建"]
        B1[文本块] --> B2[嵌入模型<br/>Embedding Model]
        B2 --> B3[向量化]
        B3 --> B4[关联元数据<br/>来源/页码]
        B4 --> B5[存入向量数据库<br/>FAISS/Chroma/Milvus]
    end

    subgraph Step3["步骤3: 检索策略优化"]
        C1[用户查询] --> C2[混合检索<br/>向量+关键词]
        C2 --> C3[初步召回]
        C3 --> C4[重排序模型<br/>Rerank]
        C4 --> C5[精选文档片段]
    end

    subgraph Step4["步骤4: 生成与提示工程"]
        D1[精选文档] --> D2[Prompt模板设计]
        D2 --> D3[LLM生成回答]
        D3 --> D4[最终答案输出<br/>有据可查]
    end
